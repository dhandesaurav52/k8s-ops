...
class IncidentManager:
    ...
    def process_pod_event(self, event_type: str, pod: Any) -> Optional[Incident]:
        ...
        # Check if there is an existing OPEN incident for this identity
        existing_incident = self.store.find_open_by_identity(identity_key)
        ...
        # -------------------------------------------------------------
            ...
            # Look for ANY open incident for this resource UID/name
            open_incidents = [
                inc for inc in self.store.list_all()
                if inc.status == "OPEN"
                and inc.resource.namespace == namespace
                and inc.resource.name == name
            ]
